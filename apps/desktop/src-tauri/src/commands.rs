...
use crate::models::{
    AudioDeviceInfo, HostdStatus, LaunchConfiguration, PlatformInfo,
};
use crate::platform;
use crate::settings;
...
pub fn get_hostd_path(app_handle: tauri::AppHandle) -> Option<String> {
    let base_dir = app_handle
        .path()
        .resource_dir()
        .unwrap_or_else(|_| std::env::current_exe().unwrap_or_default().parent().unwrap_or_else(|| std::path::Path::new(".")).to_path_buf());
...
pub async fn start_streaming(
    ...
    let hostd_path = hostd_locator::locate(&base_dir)
        .ok_or_else(|| "hostd binary not found. Build or install the bundled launcher.".to_string())?;
...
pub async fn get_process_state(
    ...
    let inner_status = process_manager
